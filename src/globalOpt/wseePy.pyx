...
cdef extern from "wsee_lambert.h":
    ...
    cdef enum Status "WSEE<4>::Status":
        Optimal, Unsolved, Infeasible

    # https://stackoverflow.com/a/41200186/8372023 
    cdef cppclass vtype "std::array<4, double>":
        double operator[](size_t)
        size_t size()

    cdef cppclass wsee4 "WSEE<4>":
        double mu[4]
        double psi[4]
        double alpha[4]
        double beta[4][4]
...
cdef class WSEE:
    ...
    def setChan(self, double[:] alpha, double[:,::1] beta):
        if not (alpha.shape[0] == 4 and alpha.ndim == 1):
            raise ValueError('alpha must be 1D array with 4 elements')

        if not (beta.shape[0] == 4 and beta.shape[1] == 4 and beta.ndim == 2):
            raise ValueError('beta must be 4x4 array')
